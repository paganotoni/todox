...
func Index(w http.ResponseWriter, r *http.Request) {
	...
	page(
		Group(
			INPUT().ID("search").TYPE("search").NAME("keyword").Attr("hx-get", "/search").Attr("hx-target", "#todoList").Attr("hx-swap", "innerHTML").Attr("hx-trigger", "keyup delay:200ms").PLACEHOLDER("Type in to search").CLASS("p-2 px-3 mb-2 w-full rounded border"),
			...
			DIV().CLASS("p-3 border bg-white items-center rounded gap-2").Children(
				FORM().Attr("hx-post", "/").Attr("hx-target", "#todoList").CLASS("flex flex-row gap-2 mb-0").Children(
